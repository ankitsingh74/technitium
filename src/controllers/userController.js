...
const loginUser = async function (req, res) {
  ...
  let token = jwt.sign(                                  //-----input({},"secret for decode"))
    {
...
const getUserData = async function (req, res) {
  let token = req.headers["x-Auth-token"];//use for case-sensitive
  if (!token) token = req.headers["x-auth-token"];//use for case-insensitive
  ...
  let decodedToken = jwt.verify(token,"secretkey");//input(token,"secretkey")
...
const updateUser = async function (req, res) {  
  ...
  try {
    ...
    console.log("aaaaaaa  "+decodedToken[0]);
